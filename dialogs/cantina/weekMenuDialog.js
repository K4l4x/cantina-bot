...
const WEEK_DAY_CHOICES = ['Montag', 'Dienstag', 'Mittwoch',
    'Donnerstag', 'Freitag'];
...
class WeekMenuDialog extends CancelAndHelpDialog {
    ...
    async prepareWeek(step) {
        ...
        return await step.prompt(WEEK_DAYS_PROMPT, {
            ...
            choices: ChoiceFactory.toChoices(WEEK_DAY_CHOICES),
    ...
    async showDayMenu(step) {
        ...
        // FIXME: Has to be done here, otherwise the dialog will not
        //  recognize cantina object and will run into an error if I want to
        //  get the menu from day (line 47-53) that the user has selected.
        ...
        const selectedDay = WEEK_DAY_CHOICES.indexOf(result) + 1;
        const menus = await cantina.menu.getDay(selectedDay);

        // TODO: Remove CardSchema and prepare the cards only once.
